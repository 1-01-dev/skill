window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function injectCss() {
      ...
      s.dataset.plugin = 'dsh-learning-coder'
      s.dataset.pluginCss = 'dsh-learning-coder'
      s.textContent = `
        ...
        .dlc-actions{display:flex;gap:7px;flex-wrap:wrap}.dlc-primary{background:#5d7cff !important;border-color:#5d7cff !important;color:#fff !important}.dlc-primary:hover:not(:disabled){background:#4f70f5 !important;border-color:#4f70f5 !important}.dlc-msg{font-size:11px;line-height:1.45;padding:8px 10px;border-radius:8px;background:rgba(93,124,255,.1);color:var(--dsw-alias-label-secondary,#b2b6c0)}.dlc-error{font-size:11px;padding:9px 10px;border-radius:8px;background:rgba(239,92,92,.1);color:#ff9696}
    ...
    function Panel({sessionId,onClose,t,prompt}) {
      ...
      const load=React.useCallback(()=>{
        ...
        api(`${BASE}/workspace?sessionId=${encodeURIComponent(sessionId)}`)
          .then(setInfo).catch(e=>setError(e.message)).finally(()=>setLoading(false))
      },[sessionId])

      React.useEffect(()=>{load()},[load])

      const openPath=(mode,path)=>api(`${BASE}/open`,{
        method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({sessionId,mode,path})
      }).catch(e=>setMessage(e.message))

      const create=async()=>{
        setCreating(true);setMessage(null)
        try {
          if(!prompt) throw new Error('Current session is unavailable')
          const result=await prompt(promptForChapter())
          if(result?.ok===false) throw new Error(result.error?.message||'Prompt rejected')
      ...
      const chapters=(w?.chapters||[]).filter(x=>x.type==='learning').slice(0,8)
      ...
      return React.createElement('section',{className:'dlc-panel',role:'dialog','aria-label':t('title')},
        React.createElement('header',{className:'dlc-head'},
          React.createElement('div',null,
            ...
            React.createElement('div',{className:'dlc-sub'},w?.name||t('loading'))),
          React.createElement('button',{className:'dlc-close',onClick:onClose,'aria-label':t('close')},'×')),
        React.createElement('div',{className:'dlc-body'},
          loading&&!info&&React.createElement('div',{className:'dlc-msg'},t('loading')),
          error&&React.createElement('div',{className:'dlc-error'},`${t('error')}: ${error}`),
          w&&React.createElement(React.Fragment,null,
            React.createElement('div',{className:'dlc-stat'},
              React.createElement('div',null,
                React.createElement('div',{className:'dlc-num'},w.chapterCount),
                React.createElement('div',{className:'dlc-label'},t('chapters'))),
              React.createElement('div',{className:'dlc-path'},w.libraryPath)),
            React.createElement('div',{className:'dlc-section'},t('recent')),
            chapters.length
              ? React.createElement('div',{className:'dlc-list'},chapters.map(c =>
                  React.createElement('div',{key:c.path,className:'dlc-item',onClick:()=>openPath('obsidian',c.path)},
                    React.createElement('div',{className:'dlc-item-title'},c.title),
                    React.createElement('div',{className:'dlc-item-meta'},[c.date,c.taskType,c.difficulty].filter(Boolean).join(' · '))
                  )
                ))
              : React.createElement('div',{className:'dlc-msg'},t('empty')),
            React.createElement('div',{className:'dlc-actions'},
              React.createElement('button',{className:'dlc-btn dlc-primary',disabled:creating,onClick:create},icon(),creating?t('creating'):t('create')),
              React.createElement('button',{className:'dlc-btn',onClick:()=>openPath('obsidian')},t('obsidian')),
              React.createElement('button',{className:'dlc-btn',onClick:()=>openPath('folder')},t('open')),
              React.createElement('button',{className:'dlc-btn',disabled:loading,onClick:load},t('refresh'))),
            message&&React.createElement('div',{className:'dlc-msg'},message)),
        )
      )
    }

    function HeaderAction({sessionId,t,prompt}) {
      const [open,setOpen]=React.useState(false)
      return React.createElement(React.Fragment,null,
        React.createElement('button',{className:'dlc-btn',onClick:()=>setOpen(true),title:t('label')},icon(),React.createElement('span',{className:'dlc-btn-label'},t('label'))),
        open&&React.createElement(Panel,{sessionId,t,prompt,onClose:()=>setOpen(false)}))
    }

    function SettingsPage({t,close}) {
      ...
      React.useEffect(()=>{
        api(`${BASE}/settings`).then(d=>setValue(d.libraryRoot||''))
      ...
      const save=()=>{
        ...
        api(`${BASE}/settings`,{
          method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({libraryRoot:value})
        }).then(d=>{setValue(d.libraryRoot||value);setSaved(true)})
          .catch(e=>setError(e.message)).finally(()=>setSaving(false))
      }

      return React.createElement('div',{className:'dlc-settings'},
        React.createElement('div',{className:'dlc-title'},t('settings')),
        React.createElement('div',{className:'dlc-field'},
          React.createElement('label',{className:'dlc-label'},t('library')),
          React.createElement('input',{className:'dlc-input',value,disabled:loading||saving,onChange:e=>setValue(e.target.value),placeholder:'~/Documents/Obsidian/Development'}),
          React.createElement('div',{className:'dlc-hint'},t('hint'))),
        React.createElement('div',{className:'dlc-actions'},
          React.createElement('button',{className:'dlc-btn dlc-primary',disabled:saving||!value.trim(),onClick:save},saving?'…':t('save')),
          saved&&React.createElement('span',{className:'dlc-hint'},t('saved')),
          close&&React.createElement('button',{className:'dlc-btn',onClick:close},t('close'))),
        error&&React.createElement('div',{className:'dlc-error'},error))
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(()=>locale.register(NS,{zh:ZH,en:EN}),'dsh-learning-coder: dictionaries')
      ...
      ctx.slots.inject('conversation.session.header.utilities',()=>ctx.slots.register({
        ...
        inject:(sessionId)=>({
          ...
          prompt:(text)=>{
            ...
            if(!binding?.session?.prompt) return Promise.reject(new Error('Current session is unavailable'))
